...
public class SearchStockActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button logOut = findViewById(R.id.logOutBtnSearchStockPage);
        ...
        Button search = findViewById(R.id.SearchForMedAllStockBtn);
        search.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                EditText u = findViewById(R.id.searchStockForMedNameOrIdAllStock);
                String input = u.getText().toString();

                if (input.isEmpty()) {
                    builder.setTitle("Search Stock").setMessage("Search parameters cannot be empty").setNeutralButton(
                            "Close",null).create().show();
                }

                else {
                    ContentValues params = new ContentValues();
                    params.put("StockSearch", input);

                    LinearLayout leftMain = findViewById(R.id.searchStockDisplayInfoLeft);
                    LinearLayout rightMain = findViewById(R.id.searchStockDisplayInfoRight);
                    leftMain.removeAllViews();
                    rightMain.removeAllViews();

                    AsyncHTTPPost asyncHTTPPost = new AsyncHTTPPost(
                            "http://lamp.ms.wits.ac.za/~s1644868/searchStock.php",params) {
                        @Override
                        protected void onPostExecute(String output) {
                            if (output.equals("Item not found")) {
                                builder.setTitle("Search Stock").setMessage("Requested item was not found").setNeutralButton(
                                        "Close",null).create().show();
                            }
                            else {
                                searchStock(output);
    ...
    public void searchStock(String output) {
        LinearLayout leftMain = findViewById(R.id.searchStockDisplayInfoLeft);
        LinearLayout rightMain = findViewById(R.id.searchStockDisplayInfoRight);
